function [Test]=OutlierTest(data,j,k,f,NonNegativity,Stepwise)

%The OutlierTest function can be used to look for outliers in the data. 
%These can be outlier samples or wavelengths. For example
%are there specific wavelengths that are weighting the model too much?
%This is done by running some simple models (not too many
%factors/components). If the dataset is very large one can chose to select
%every other Emission wavelength, (i.e. 300, 304, 306 ....nm, instead of
%the measured 300, 302, 304, 306....).
%
%INPUT: [Test]=OutlierTest(data,j,k,f,NonNegativity,Stepwise)
%
%data: identifies the data to be modelled (e.g. OriginalData).
%j: Emission wave frequency selection (j=1 means it will take all the data, 
%    j=2 .. every other wavlength etc)
%k: Excitation wave frequency selection (Same as above but for excitation data)
%f: Max number of factors to model during the Test. First will be 2, then
%   it will continue up to f.
%NonNegativity : Use non-negatvivty constraints? Write 'Yes' or 'No'
%Stepwise : Run models stepwise 'Yes' or 'No'. If 'Yes' then outliertest 
%   models are run one at a time (after pressing enter) and diagnostic 
%   plots are created immediately after each PARAFAC model. 
%Tip: Use Ctrl-f4 to close a graph window. 
%If Stepwise is 'Yes' then press enter to continue to the next PARAFAC model. 
%
%Example
%[Test1]=OutlierTest(CutData,4,2,6,'No','No')
%or
%[Test2]=OutlierTest(Test2,1,1,6,'Yes','No')
...
Test.X=data.X(:,1:(j):(data.nEm),1:(k):(data.nEx));
Test.Em=data.Em(1:(j):(data.nEm));
Test.Ex=data.Ex(1:(k):(data.nEx));
Test.nEm=size(Test.Em,1);
Test.nEx=size(Test.Ex,1);
Test.nSample=size(Test.X,1);


switch NonNegativity
    case{'Yes'}
        switch Stepwise
            case{'Yes'}
                for NumFac=2:f, (pause)
                [Model]=parafac(Test.X,[NumFac],[1e-2 1 1 1],[2 2 2]);
                eval(['Test.Model',int2str(NumFac),'=Model']);
                end
        end
end
...
switch NonNegativity
    case{'No'}
        switch Stepwise
            case{'Yes'}
                for NumFac=2:f, (pause)
                [Model]=parafac(Test.X,[NumFac],[1e-2 1 1 1]);
                eval(['Test.Model',int2str(NumFac),'=Model']);
                end
        end
end
...
switch NonNegativity
    case{'Yes'}
        switch Stepwise
            case{'No'}
                for NumFac=2:f, 
                [Model]=parafac(Test.X,[NumFac],[1e-2 1 0 1],[2 2 2]);
                eval(['Test.Model',int2str(NumFac),'=Model']);
                end
        end
end
