...
function [data]=RandInitAnal(data,f,it)

%This function runs a series of PARAFAC models using random initialisation.
%INPUT: [data]=RandInitAnal(data,f,it)
%data: data structure to read and save to.
...
[Model,Iter,Err]=parafac(data.X,f,[1e-6 2],[2 2 2]);

eval(['data.Model',int2str(f),'_It',int2str(i),'=Model',]);
eval(['data.Err',int2str(f),'_It',int2str(i),'=Err',]);
eval(['data.Iter',int2str(f),'_It',int2str(i),'=Iter',]);
