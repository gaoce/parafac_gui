function [AnalysisData]=SplitData2(data)

% The SplitData2 function splits the data into halves in preperation for the
% Split Half Analysis. Offers an alternative split to SplitData
%
...
temp1=AnalysisData.X(1:(round((AnalysisData.nSample)/4))-1,:,:);
temp2=AnalysisData.X((round((AnalysisData.nSample)/4)):(round((AnalysisData.nSample)/2))-1,:,:);
temp3=AnalysisData.X((round((AnalysisData.nSample)/2)):(round((AnalysisData.nSample)/4*3))-1,:,:);
temp4=AnalysisData.X((round((AnalysisData.nSample)/4*3))-1:(AnalysisData.nSample),:,:);
