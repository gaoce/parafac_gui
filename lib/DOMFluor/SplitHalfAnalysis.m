function [AnalysisData]=SplitHalfAnalysis(AnalysisData,f,SaveDataAs)

%The SplitHalfAnalysis function carries out two split half analyses on the
%four halves created by the SplitData function.
%
%INPUT: [data]=SplitHalfAnalysis(data,f,SaveDataAs)
%data: refers to the data structure where the data is (e.g. AnalysisData)
%f: Defines the minimum number of components to fit. This can either be a
%       number or a range (e.g. 3 or 3:5, to fit a 3, 4 then 5 component model)
%SaveDataAs: Type file name where the workspace to be saved as.
%       Data is saved automatically as the models are fitted, so that
%       the function can be stopped prematurely and all data is not lost.
%
%Example
%[AnalysisData]=SplitHalfAnalysis(AnalysisData,(2:6),'tutorial.mat')
...
for NumFac=(f)
    [Fac,It,Err]=parafac(AnalysisData.Split(1).X,NumFac,[],[2 2 2]);
    eval(['AnalysisData.Split(1).Fac_',int2str(NumFac),'=Fac']); 
    eval(['AnalysisData.Split(1).It_',int2str(NumFac),'=It']);
    eval(['AnalysisData.Split(1).Err_',int2str(NumFac),'=Err']);
    eval(['save ',SaveDataAs]);
    
   [Fac,It,Err]=parafac(AnalysisData.Split(2).X,NumFac,[],[2 2 2]);
    eval(['AnalysisData.Split(2).Fac_',int2str(NumFac),'=Fac']); 
    eval(['AnalysisData.Split(2).It_',int2str(NumFac),'=It']);
    eval(['AnalysisData.Split(2).Err_',int2str(NumFac),'=Err']);
    eval(['save ',SaveDataAs]);
    
    [Fac,It,Err]=parafac(AnalysisData.Split(3).X,NumFac,[],[2 2 2]);
    eval(['AnalysisData.Split(3).Fac_',int2str(NumFac),'=Fac']); 
    eval(['AnalysisData.Split(3).It_',int2str(NumFac),'=It']);
    eval(['AnalysisData.Split(3).Err_',int2str(NumFac),'=Err']);
    eval(['save ',SaveDataAs]);
    
    [Fac,It,Err]=parafac(AnalysisData.Split(4).X,NumFac,[],[2 2 2]);
    eval(['AnalysisData.Split(4).Fac_',int2str(NumFac),'=Fac']); 
    eval(['AnalysisData.Split(4).It_',int2str(NumFac),'=It']);
    eval(['AnalysisData.Split(4).Err_',int2str(NumFac),'=Err']);
    eval(['save ',SaveDataAs]);
end
